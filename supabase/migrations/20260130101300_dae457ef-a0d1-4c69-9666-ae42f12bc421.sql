-- Add assigned_email column to access_codes table
ALTER TABLE public.access_codes 
ADD COLUMN assigned_email TEXT;

-- Add a comment explaining the column
COMMENT ON COLUMN public.access_codes.assigned_email IS 'Email address of the intended recipient. Code can only be redeemed by a user with this email.';

-- Update the redeem_access_code function to validate email
CREATE OR REPLACE FUNCTION public.redeem_access_code(p_code TEXT, p_discord_id TEXT)
...
BEGIN
  ...
  IF v_code_record.assigned_email IS NOT NULL AND v_code_record.assigned_email != '' THEN
    IF LOWER(v_user_email) != LOWER(v_code_record.assigned_email) THEN
      RETURN jsonb_build_object(
        'success', false,
