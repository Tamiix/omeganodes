...
const CryptoPaymentModal = ({ isOpen, onClose, amount, commitment, rps = 100, tps = 50, serverType = "shared", location = "all", rentAccessEnabled = false, isTestMode = false, discordUserId = "", appliedDiscount = null, includeShredsFromPricing = false, additionalStakePackages = 0, initialReferralCode }: CryptoPaymentModalProps) => {
  ...
  const [referralInfo, setReferralInfo] = useState<{ referrer_id: string; referrer_username: string } | null>(null);
  ...
  const validateReferralCode = async () => {
    ...
    try {
      ...
      const result = data?.[0];
      if (!result?.is_valid) {
        setReferralError(result?.error_message || 'Invalid referral code');
        setReferralInfo(null);
        return;
      }
      setReferralInfo({ referrer_id: result.referrer_id, referrer_username: result.referrer_username });
    } catch (err) {
            ...
            if (!orderError && orderData && referralInfo) {
              ...
              try {
                await supabase.from('referrals').insert({
                  ...
                  order_amount_usd: totalAmount,
                  commission_amount: commissionAmount,
                  commission_rate: 0.10,
                  status: 'confirmed'
                });
              } catch (refErr) {
                console.error("Failed to save referral:", refErr);
              }
            }
          } catch (dbErr) {
